...
async def cmd_start(message: Message, bot: Bot):
    ...
    if candidate.stage == 0:
        # Первый запуск — разветвление по позиции
        if getattr(candidate, "position", "support") == "sales":
            # Воронка продажника
            from handlers.sales import start_sales_flow
            await start_sales_flow(candidate, message, bot)
            await notify_hr(bot, f"✅ Кандидат <b>{candidate.full_name}</b> (МП) активировал приглашение.")
        else:
            # Воронка специалиста по сопровождению (как раньше)
            async with get_session() as s:
