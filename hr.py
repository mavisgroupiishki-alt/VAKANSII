...
from data.keyboards import (
    ...
    kb_internship_start_dates, kb_direct_internship_day_choice,
)
...
async def cmd_help(message: Message):
    ...
    text = (
        ...
        "/add_intern — добавить кандидата сразу на стажировку\n"
...
async def add_start(message: Message, state: FSMContext):
    if not is_hr(message.from_user.id):
        return
    await state.clear()
    await state.update_data(direct_internship=False)
...
@router.message(Command("add_intern"))
async def direct_internship_add_start(message: Message, state: FSMContext):
    """Добавить продажника сразу на стажировку и выдать ссылку-приглашение."""
    if not is_hr(message.from_user.id):
        return
    await state.clear()
    await state.update_data(direct_internship=True)
    await state.set_state(AddCandidate.waiting_full_name)
    await message.answer(
        "<b>🚀 Добавление кандидата сразу на стажировку</b>\n\n"
        "Этот сценарий пропускает анкету, видео и стартовый тест. "
        "После перехода по ссылке кандидат сразу получит выбранный стажировочный день.\n\n"
        "Введите <b>ФИО кандидата</b>:\n\n"
        "(или /cancel для отмены)"
    )
...
async def add_source(callback: CallbackQuery, state: FSMContext):
    ...
    if data.get("direct_internship"):
        await state.set_state(AddCandidate.waiting_internship_day)
        await callback.message.answer(
            "📌 С какого стажировочного дня начать кандидата?",
            reply_markup=kb_direct_internship_day_choice(),
        )
...
@router.callback_query(AddCandidate.waiting_internship_day, F.data.startswith("direct_day_"))
async def add_direct_internship_day(callback: CallbackQuery, state: FSMContext, bot: Bot):
    """Создать кандидата-продажника сразу на выбранный день стажировки."""
    try:
    ...
    if start_day not in (1, 2, 3):
        await callback.answer("Можно выбрать только День 1, День 2 или День 3.", show_alert=True)
        return

    data = await state.get_data()
    invite_code = _gen_invite_code()
    sales_data = json.dumps({
        "direct_internship": True,
        "direct_internship_day": start_day,
        "added_directly_to_internship_at": datetime.utcnow().isoformat(),
    }, ensure_ascii=False)
    ...
    async with get_session() as s:
        ...
        candidate = Candidate(
            ...
            status="active",
            added_by=callback.from_user.id,
            awaiting="direct_internship_invite_pending",
            internship_day=0,
            internship_step="direct_invite_pending",
    ...
    await callback.message.answer(
        f"✅ Кандидат <b>{html.escape(data['full_name'])}</b> добавлен сразу на стажировку (#{cid}).\n"
        f"Источник: <b>{source_label(data.get('source', ''))}</b>\n"
        f"Позиция: <b>💼 Менеджер по продажам</b>\n"
        f"Стартовый день: <b>День {start_day}</b>\n\n"
        f"<b>📨 Ссылка-приглашение:</b>\n"
        f"<code>{invite_link}</code>\n\n"
        f"Отправьте ссылку кандидату. Когда он перейдёт по ней, бот пропустит анкету, видео и стартовый тест "
        f"и сразу отправит ему <b>День {start_day} стажировки</b>."
    )
    await callback.answer()
...
@router.message(F.text == "🚀 Добавить сразу на стажировку")
async def menu_add_direct_internship_candidate(message: Message, state: FSMContext):
    """Добавление кандидата сразу в стажировку, без анкеты и стартового теста."""
    await direct_internship_add_start(message, state)
